...
points2 = np.array([[9,-1],[11,-1],[11,1],[9,1]], dtype=np.float64) + np.array([-5,0], dtype=np.float64)
...
aprox_square_sqrt = lambda x,y : aprox_sqrt(A_val[0,0]*x + A_val[0,1]*y - b_val[0]) + aprox_sqrt(A_val[1,0]*x + A_val[1,1]*y - b_val[1]) + aprox_sqrt(A_val[2,0]*x + A_val[2,1]*y - b_val[2]) + aprox_sqrt(A_val[3,0]*x + A_val[3,1]*y - b_val[3])
...
in_inds = aprox_square_sqrt(X,Y)/(n_ineq*np.sqrt(eps)) <= alpha_sol
...
ax.add_line(get_line(points2[2], points2[3]))
