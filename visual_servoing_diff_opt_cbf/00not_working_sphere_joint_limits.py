...
def main():
    ...
    cons = [cp.sum(cp.exp(kappa*(_A_target @ _p - _b_target))) <= nc_target*_alpha, 
            cp.sum_squares(_Q_sqrt_obstacle @ _p)+ _q_obstacle.T @ _p + _c_obstacle + 1 <= _alpha]
    # cons = [cp.sum(cp.exp(kappa*(_A_target @ _p - _b_target))) <= nc_target*_alpha, 
    #         cp.exp(kappa*(cp.sum_squares(_Q_sqrt_obstacle @ (_p-p_offset))+ _q_obstacle.T @ (_p-p_offset) + _c_obstacle)) <= _alpha]
   
    problem = cp.Problem(obj, cons)
    assert problem.is_dpp()

    cvxpylayer = CvxpyLayer(problem, parameters=[_A_target, _b_target, _Q_sqrt_obstacle, _q_obstacle, _c_obstacle], variables=[_alpha, _p], gp=False)
    ...
    for i in range(T):
        ...
        if i % step_every == 0:
            ...
            if CBF_config["active"] == 1:
                ...
                p_offset_tensor = torch.tensor(p_offset)
                ...
                q_val_tmp = torch.tensor([-2*xc*zc*fx, -2*yc*zc*fx])
                q_val = q_val_tmp - 2*Q_obstacle_val @ p_offset_tensor
                c_val_tmp = torch.tensor([fx**2*(xc**2+yc**2-sphere_radius**2)])
                c_val = c_val_tmp + p_offset_tensor @ Q_obstacle_val @ p_offset_tensor - q_val_tmp @ p_offset_tensor

                # time1 = time.time()
                alpha_sol, p_sol = cvxpylayer(A_target_val, b_target_val, Q_sqrt_val, q_val, c_val)
                ...
                for ii in range(camera_config["width"]):
                    for jj in range(camera_config["height"]):
                        ...
                        if np.sum(np.exp(kappa * (A_target_val @ pp - b_target_val))) <= 4:
                            x, y = pp
                            img = cv2.circle(img, (int(x),int(y)), radius=5, color=(0, 0, 255), thickness=-1)
                        if np.sum((Q_sqrt_val @ pp)**2) + q_val.T @ pp + c_val <=0:
                            x, y = pp
                            img = cv2.circle(img, (int(x),int(y)), radius=5, color=(0, 0, 255), thickness=-1)
