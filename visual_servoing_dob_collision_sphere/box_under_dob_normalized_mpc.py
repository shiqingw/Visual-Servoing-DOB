...
def main():
    ...
    results_dir = "{}/results_collision_dob_sphere/exp_{:03d}".format(str(Path(__file__).parent.parent), exp_num)
    ...
    for i in range(T):
        ...
        if i % step_every == 0:
            ...
            H_jl_qp = np.eye(9)
            g_jl_qp = - dq_nominal
            C_jl_qp = np.eye(9)*dt*step_every
            joint_limits_qp.update(H=H_jl_qp, g=g_jl_qp, l=joint_lb - q, u=joint_ub - q, C=C_jl_qp)
