...
if __name__ == '__main__':
    ...
    for i in range(len(exp_names)):
        ...
        cbf_value = np.min(summary["cbf_values"], axis=1)
        cbf_value[stop_ind:] = None

        plt.plot(times, cbf_value, linestyle = linestyles[i], color = 'tab:green',
                linewidth = linewidth, label="$h_{min}$" + " ("+labels[i]+")")
    ...
    plt.tight_layout()
    plt.draw()
    results_dir = "{}/results_collision_dob_sphere/exp_{:03d}".format(str(Path(__file__).parent.parent), 0)
