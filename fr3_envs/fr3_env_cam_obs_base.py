...
class FR3CameraSim(Env):
    ...
    def __init__(self, camera_config, enable_gui_camera_data, base_translation=[0,0,0], base_orientation_euler=[0,0,0], obs_urdf="box.urdf", render_mode: Optional[str] = None, record_path=None):
        ...
        base_p_offset = (np.array(_base_pos)+ np.array(base_translation, dtype=np.float32)).reshape(-1,1)
    ...
    def get_info(self, q, dq):
        ...
        p_hand, R_hand, q_HAND = self.compute_crude_location(
            np.eye(3), np.array(([0.0], [0.0], [0.02])), self.FR3_HAND_FRAME_ID, self.base_R_offset, self.base_p_offset
        )
